...
とりあえず日本語表示を試すだけなら、有志の方が7月25日時点のbigファイルを作成してくれたのでこちらのほうが簡単です。

・【宇宙】 Homeworld 総合スレ 【戦略】 Stage 10 [転載禁止]c2ch.net
...
作業ステータス  
* 作業中: 途中まで作業している最中です。
* ほぼ完了: 翻訳はだいたい終わっているが、一部変わらない部分が残っている場合です。未翻訳の部分を翻訳していただけると助かります。
* 完了（未チェック）: 翻訳は終了しているが、実際のゲーム画面で表示のチェックをまだしていない状態。
* 完了: ゲーム画面でのチェックも終わった状態。

なお、実際のゲーム内で使われていないデータ部分は翻訳していません。

### 着手

* events.dat		完了（未チェック）
* fontmap.lua		※翻訳対象外。フォントファイル名を変更してある
